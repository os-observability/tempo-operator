...
type TenantsSpec struct {
	...
	// +operator-sdk:csv:customresourcedefinitions:type=spec,xDescriptors={"urn:alm:descriptor:com.tectonic.ui:select:dynamic","urn:alm:descriptor:com.tectonic.ui:select:openshift"},displayName="Mode"
...
// OPASpec defines the opa configuration spec for Tempo Gateway component.
type OPASpec struct {
...
type ObjectStorageTLSSpec struct {
	...
	// It needs to be in the same namespace as the (Tempo) Microservices custom resource.
	//
	// +optional
	// +kubebuilder:validation:optional
